...
public abstract class UntapForTurnAction {
    public final static Class<UntapStepBeginEvent> type = UntapStepBeginEvent.class;
    public final static EventListener turnBasedAction = new EventListener() {
        @Override
        public void accept(MTGGame game, Event event) {
            List<Card> cards = game.data.getCardsByStatus(CARD_STATUS_TYPE.TAPPED, true);
            for (Card c : cards) {
